...
def crop_boxes(detections, image, colors):
    import cv2
    crops=[]
    if (len(detections)!=0) and (image is not None):
        for label, confidence, bbox in detections:
            ...
            height, width, channels = croped.shape
            new_width=140
            new_hight=140
            try:
                croped = cv2.resize(croped, dsize=(new_width, new_hight), interpolation=cv2.INTER_CUBIC)
            except:
                return []
            crops.append(croped)
    return crops
